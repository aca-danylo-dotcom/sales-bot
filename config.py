...
# --- Оплата картой внутри Telegram (Telegram Payments) ---
# Токен выдаёт BotFather после подключения платёжного провайдера: /mybots →
# бот → Payments. Пока он не задан, магазин работает по-старому — переводом на
# карту с кнопкой «Я оплатил». Это не запасной путь на случай ошибки, а рабочий
# режим: провайдер есть не у всех и подключается не за день.
PAYMENT_PROVIDER_TOKEN: str = os.getenv("PAYMENT_PROVIDER_TOKEN", "").strip()
# Валюта счёта по стандарту ISO 4217. SHOP_CURRENCY рядом — это подпись для
# человека («грн»), Telegram же принимает только код и от подписи его надо
# отличать: перепутав их, счёт не выставить вовсе.
PAYMENT_CURRENCY_CODE: str = os.getenv("PAYMENT_CURRENCY_CODE", "UAH").strip().upper()


def payments_enabled() -> bool:
    """Можно ли выставить счёт в Telegram. Всё остальное — оплата на карту."""
...
# Публичный адрес, по которому открывается витрина. Telegram принимает ТОЛЬКО
# https и только настоящий домен: localhost и http он не откроет — проверить
# мини-приложение можно лишь на выложенном сервере или через туннель.
#
# Пусто — кнопки магазина в боте не появляются, и всё работает как раньше,
# перепиской. Это нормальный режим: пока приложение не выложено, кнопка вела бы
# в никуда.
WEBAPP_URL: str = os.getenv("WEBAPP_URL", "").strip().rstrip("/")


def webapp_enabled() -> bool:
    """Есть ли куда вести кнопку магазина."""
    return WEBAPP_URL.startswith("https://")


# --- Вход в панель из браузера ---
# Пароль на веб-CRM. Внутри Telegram он не нужен и не спрашивается: там человека
# опознаёт сам мессенджер по подписи (см. web/auth.py). Пароль закрывает ровно
# одну дверь — прямой заход по адресу панели из браузера.
#
# ПУСТО = ПАНЕЛЬ ОТКРЫТА ВСЕМ, кто знает адрес: заказы, телефоны и адреса
# клиентов. Так было раньше, и для запуска на своём ПК это удобно. На хостинге
# оставлять пустым нельзя — поддомены вроде *.up.railway.app находят перебором.
CRM_PASSWORD: str = os.getenv("CRM_PASSWORD", "")

# Сколько дней держится вход. Панель открывают каждый день, и просить пароль
# чаще, чем раз в месяц, значит приучить человека держать его в заметке.
CRM_SESSION_DAYS: int = max(1, int(os.getenv("CRM_SESSION_DAYS", "30")))


def crm_password_set() -> bool:
    """Закрыт ли браузерный вход паролем."""
    return bool(CRM_PASSWORD)


# --- Веб-CRM ---
# Без CRM_PASSWORD панель работает БЕЗ входа: кто открыл адрес — тот внутри.
