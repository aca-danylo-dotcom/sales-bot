...
# Пароль сюда приходит уже хешированным (web/security.py): открытый пароль не
# должен доходить до слоя базы вообще, чтобы его нечем было записать по ошибке.


async def create_user(login: str, password_hash: str, display_name: str = "") -> int | None:
    """Заводит пользователя CRM. None, если такой логин уже занят."""
    async with get_connection() as conn:
        cursor = await conn.execute(
            "INSERT OR IGNORE INTO users (login, password_hash, display_name, created_at) "
            "VALUES (?, ?, ?, ?)",
            (login.strip().lower(), password_hash, display_name.strip(), config.now_str()),
        )
        await conn.commit()
...
async def get_user_by_login(login: str) -> dict | None:
    """Пользователь по логину — включая отключённого: решение о доступе не здесь."""
    async with get_connection() as conn:
        cursor = await conn.execute(
...
async def list_users() -> list[dict]:
    async with get_connection() as conn:
        cursor = await conn.execute("SELECT * FROM users ORDER BY login")
        return _rows(await cursor.fetchall())
...
async def set_user_active(user_id: int, is_active: bool) -> None:
    """Отключает вход, не удаляя пользователя.

    Удалять нельзя: на нём висят взятые в работу заказы (orders.assignee), и
    после удаления непонятно, кто их вёл.
    """
    async with get_connection() as conn:
        await conn.execute(
