...
# ─────────────────── Доступы в веб-CRM ───────────────────
# Пользователей панели заводит только владелец и только отсюда: страницы
# «зарегистрироваться» в CRM нет вовсе, поэтому подобрать её адрес и завести
# себе доступ снаружи невозможно.


_CRM_HELP = (
    "🔐 <b>Доступы в веб-CRM</b>\n\n"
    "<code>/crm_add логин пароль Имя</code> — завести менеджера\n"
    "<code>/crm_pass логин новый_пароль</code> — сменить пароль\n"
    "<code>/crm_off логин</code> — закрыть доступ\n"
    "<code>/crm_on логин</code> — вернуть доступ\n\n"
    "Пароль — минимум 8 символов. Сообщение с паролем бот удаляет из чата сразу "
    "после выполнения: переписка хранится на телефоне и в облаке Telegram."
)

_MIN_PASSWORD = 8


async def _forget_command(message: Message) -> None:
    """Удаляет сообщение владельца с паролем из чата.

    Не срабатывает, если сообщение старше 48 часов — Telegram такие удалять не
    даёт. Это не повод падать: пароль всё равно уже сменён.
    """
    try:
        await message.delete()
    except TelegramBadRequest:
        logger.info("Сообщение с паролем удалить не удалось (слишком старое).")


def _crm_args(message: Message, count: int) -> list[str] | None:
    """Разбирает аргументы команды. None — если их не столько, сколько нужно."""
    parts = (message.text or "").split(maxsplit=count)
    return parts[1:] if len(parts) >= count + 1 else None


@router.message(Command("crm"))
async def cmd_crm(message: Message, state: FSMContext) -> None:
    await state.clear()
    users = await queries.list_users()
    lines = [_CRM_HELP]
    if users:
        lines.append("\n<b>Кто заведён:</b>")
        for user in users:
            mark = "✅" if user["is_active"] else "🚫"
            name = f" — {_esc(user['display_name'])}" if user["display_name"] else ""
            lines.append(f"{mark} <code>{_esc(user['login'])}</code>{name}")
    else:
        lines.append("\nПока никого нет.")
...
async def cmd_crm_add(message: Message, state: FSMContext) -> None:
    await state.clear()
    args = _crm_args(message, 2)
    if not args:
        await _answer(message, "Нужно: <code>/crm_add логин пароль Имя</code>")
        return

    login, rest = args[0], args[1].split(maxsplit=1)
    password = rest[0]
    display_name = rest[1] if len(rest) > 1 else ""
    await _forget_command(message)

    if len(password) < _MIN_PASSWORD:
        await _answer(message, f"Пароль слишком короткий — нужно от {_MIN_PASSWORD} символов.")
        return

    user_id = await queries.create_user(
        login, security.hash_password(password), display_name
    )
    if user_id is None:
        await _answer(message, f"Логин <code>{_esc(login)}</code> уже занят.")
        return
    await _answer(
        message,
        f"✅ Доступ создан: <code>{_esc(login.lower())}</code>.\n"
        "Пароль передайте человеку сами — бот его больше не покажет.",
    )


@router.message(Command("crm_pass"))
async def cmd_crm_pass(message: Message, state: FSMContext) -> None:
    await state.clear()
    args = _crm_args(message, 2)
    if not args:
        await _answer(message, "Нужно: <code>/crm_pass логин новый_пароль</code>")
        return

    login, password = args[0], args[1].split(maxsplit=1)[0]
    await _forget_command(message)
    ...
    user = await queries.get_user_by_login(login)
    if not user:
        await _answer(message, f"Логина <code>{_esc(login)}</code> нет. Список: /crm")
        return

    await queries.set_user_password(user["id"], security.hash_password(password))
    await _answer(message, f"✅ Пароль для <code>{_esc(user['login'])}</code> изменён.")


async def _switch_access(message: Message, state: FSMContext, *, active: bool) -> None:
    await state.clear()
    args = _crm_args(message, 1)
    if not args:
        command = "crm_on" if active else "crm_off"
        await _answer(message, f"Нужно: <code>/{command} логин</code>")
        return

    user = await queries.get_user_by_login(args[0].split()[0])
    if not user:
        await _answer(message, f"Логина <code>{_esc(args[0])}</code> нет. Список: /crm")
        return

    await queries.set_user_active(user["id"], active)
    # Открытая сессия закрывается на первом же клике: middleware проверяет
    # is_active при каждом запросе, а не только при входе.
    status = "открыт" if active else "закрыт"
    await _answer(message, f"Доступ для <code>{_esc(user['login'])}</code> {status}.")


@router.message(Command("crm_off"))
async def cmd_crm_off(message: Message, state: FSMContext) -> None:
    await _switch_access(message, state, active=False)


@router.message(Command("crm_on"))
async def cmd_crm_on(message: Message, state: FSMContext) -> None:
    await _switch_access(message, state, active=True)
