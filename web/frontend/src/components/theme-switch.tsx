 ...
 *
 * Компонент «Switch Button» из реестра kokonutui (@dorianbaffier, MIT,
 * kokonutui.com) — владелец прислал его код и попросил именно этот вид.
 * Оформление сохранено как есть: солнце поворачивается на 360° под курсором,
 * подпись Light/Dark проявляется через прозрачность, по кнопке пробегает
 * блик.
 *
 * Изменено ровно одно: источник темы. В оригинале это `useTheme` из
 * next-themes, а Next.js у нас нет и провайдера тоже — вместо него наш хук
 * (см. lib/theme.ts), он держит тему классом на <html> и помнит выбор.
 *
 * Ещё одна мелочь не для красоты: подпись «Light/Dark» на узком экране
 * убирается (`showLabel`), потому что рядом с названием магазина и кнопкой
 * меню на телефоне места нет.
 */
...
export default function ThemeSwitch({
  ...
  return (
    <Button
      ...
      className={cn(
        // `overflow-hidden` — не украшение: блик ждёт своей очереди в
        // `translate-x-[-100%]`, то есть ЛЕВЕЕ кнопки, и без обрезки по её
        // рамке он просто висит в шапке светлой полосой. В тёмной теме это
