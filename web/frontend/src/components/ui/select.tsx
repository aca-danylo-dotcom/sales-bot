/**
 * Выпадающий список из реестра shadcn/ui — страница «Select» (вариант React Aria).
 *
 * Зачем он вместо обычного <select>. Системный список рисует браузер, и
 * договориться с ним нельзя: на этом уже обожглись — правила оформления списка
 * в старой панели однажды сделали его прозрачным, и по всей CRM «кнопки
 * перестали работать». Здесь список — обычная разметка: он выглядит одинаково
 * в любом браузере и ломаться от обновления браузера ему нечем.
 *
 * Взято с https://ui.shadcn.com/docs/components/aria/select с тремя правками:
 *
 * 1. Значки — из lucide (в реестре на их месте заглушка, которую подставляет
 *    их установщик под выбранный набор значков).
 * 2. Убран `SelectInput` — поиск внутри списка. Он тянет за собой ещё один
 *    компонент реестра, а списки в панели короткие: категорий десяток.
 * 3. Пути импорта под наш проект.
 *
 * Клавиатура, чтение с экрана и закрытие по щелчку мимо — на React Aria,
 * своего кода здесь нет.
 */
