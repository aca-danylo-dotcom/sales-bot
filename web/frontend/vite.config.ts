...
export default defineConfig({
  ...
  /* Прод-сборка серверного рендеринга не использует: панель рисует браузер.
     Настройка нужна для проверки — разделы прогоняются через renderToString на
     настоящих данных, чтобы поймать ошибку отрисовки до того, как её увидит
     менеджер. Часть библиотеки графиков собрана как CommonJS, и без noExternal
     такой прогон падает на импорте. На обычную сборку это не влияет. */
  ssr: {
    noExternal: true,
  },
  server: {
